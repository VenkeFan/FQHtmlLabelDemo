...
- (void)layoutSubviews {
    ...
    _htmlParser.imagePaddingY = self.imagePaddingY;
}
...
- (void)p_drawAttributedText:(NSAttributedString *)attributedText {
    ...
    dispatch_async(_renderQueue, ^{
        if (self->_ctFramesetter) CFRelease(self->_ctFramesetter);
        self->_ctFramesetter = CTFramesetterCreateWithAttributedString((__bridge CFAttributedStringRef)attributedText);
        
        CGSize bounds = CTFramesetterSuggestFrameSizeWithConstraints(self->_ctFramesetter, CFRangeMake(0, attributedText.length), nil, CGSizeMake(width, CGFLOAT_MAX), nil);
        ...
        if (self->_ctFrame) CFRelease(self->_ctFrame);
        self->_ctFrame = CTFramesetterCreateFrame(self->_ctFramesetter, CFRangeMake(0, 0), path, NULL);
        
        CFArrayRef lines = CTFrameGetLines(self->_ctFrame);
        CFIndex lineCount = CFArrayGetCount(lines);
        
        CGPoint lineOrigins[lineCount];
        CTFrameGetLineOrigins(self->_ctFrame, CFRangeMake(0, 0), lineOrigins);
        
        {
            ...
            for (CFIndex index = 0; index < lineCount; index++) {
                ...
                for (CFIndex i = 0; i < CFArrayGetCount(runs); i++) {
                    ...
                    runBounds.origin.y = ctLineOrigin.y - descent + self.imagePaddingY;
                    runBounds.size.height -= (self.imagePaddingY * 2);
...
//                    CGContextSetAlpha(context, 0.2);
            ...
            if (self->_ctFramesetter) {
                CFRelease(self->_ctFramesetter);
                self->_ctFramesetter = nil;
            }
        }
...
- (void)setImagePaddingY:(CGFloat)imagePaddingY {
    _imagePaddingY = imagePaddingY;
    
    _htmlParser.imagePaddingY = imagePaddingY;
}
